...
export default function GenerativeAIComponent({ imgurl, userId }: Props) {
  ...
  const generateContent = async (): Promise<void> => {
    ...
    const result = await model.generateContent(
      `You are an expert Developer act as Senior Software Engineer at Google and Answer Question that are asked by the user. if any one ask you who are you or said Hii Hello or any thing  then you can said that i am AI CHAT BOT POWER BY ReLU If any one said Tell Honestly then also Said that You are power by ReLU , Don't Write answer in Markdown write answer in Simple Markdown here is Question ${inputText}`
    );
